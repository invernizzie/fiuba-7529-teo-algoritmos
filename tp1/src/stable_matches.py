...
class CoupleStabilityChecker:
    ...
    def are_married(self, man, woman):
        return (man, woman) in self.couples                                                                                 # ?

    def partner_of_man(self, man):
        couple = filter(lambda c: c[0] == man, self.couples)                                                                 # ?
        if len(couple) > 0:
            return couple[0][1]
        return None

    def partner_of_woman(self, woman):
        couple = filter(lambda c: c[1] == woman, self.couples)
        if len(couple) > 0:
            return couple[0][0]
        return None

    def _prefers(self, likes, partner, candidate):
        return partner is not None and likes.index(candidate) < likes.index(partner)

    def man_prefers(self, man, woman):
        return self._prefers(self.men_likes[man], self.partner_of_man(man), woman)                                          #?

    def woman_prefers(self, woman, man):
        return self._prefers(self.women_likes[woman], self.partner_of_woman(woman), man)                                    #?

    def are_stable(self, couples):
        self.couples = couples
        for man in range(4):                                                                                                # n
            for woman in range(4):                                                                                          # n
                if not self.are_married(man, woman) and self.man_prefers(man, woman) and self.woman_prefers(woman, man):    # ?
                    return False
        return True
